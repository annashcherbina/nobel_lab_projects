#module load homer
#scanMotifGenomeWide.pl myod.motif mm10 -p 10 -bed > myod.scan.out.bed 
#intersect peaks with motifs
#for f in `cat ../overlap_peaks.txt`
#do
#    f_base=`basename $f`
#    #bedtools intersect -a myod.scan.out.bed -b $f > $f_base.myod.intersection.bed
#    #split by +/- strand
#    grep "-" $f_base.myod.intersection.bed > $f_base.myod.intersection.neg.bed
#    grep -v "-" $f_base.myod.intersection.bed > $f_base.myod.intersection.pos.bed
#    
#    #pad motif intesections to 200 bp
#    python ~/anna_utils/seq_utils/pad.py --input_bed $f_base.myod.intersection.pos.bed --desired_length 200 --output_bed $f_base.myod.intersection.pos.padded.bed --chromsizes ../mm10.chrom.sizes
#    python ~/anna_utils/seq_utils/pad.py --input_bed $f_base.myod.intersection.neg.bed --desired_length 200 --output_bed $f_base.myod.intersection.neg.padded.bed --chromsizes ../mm10.chrom.sizes    
#done
