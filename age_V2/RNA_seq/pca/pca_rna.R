...
data=data.frame(read.table('../age.rna.counts.txt.expected_count',header=TRUE,sep='\t',row.names=1))
data$GeneName=NULL 
data=data[rowSums(data)>0,]

data=as.matrix(data)
getVar <- apply(data[, -1], 1, var)
data=data[getVar>0,]
batches=data.frame(read.table("../batches.tsv",header=TRUE,sep='\t'))
...
mod1=model.matrix(~Day+Age+Batch+Pax7,data=batches)
...
fit <- lmFit(v)
sv_contribs=coefficients(fit)[,7:13] %*% t(fit$design[,7:13])
v$E=v$E-sv_contribs

spearman_cor=cor(v$E,method="spearman")
write.csv(spearman_cor,"rna.spearman_cor.csv")
pearson_cor=cor(v$E,method="pearson")
write.csv(pearson_cor,"rna.pearson_cor.csv")

data.pca=prcomp(t(v$E),scale=FALSE,center=FALSE)
var_explained=as.character(round(100*data.pca$sdev^2/sum(data.pca$sdev^2),2))

png('rna.varexplained.png',height=10,width=10,units='in',res=300)
...
cont.matrix=makeContrasts(d0_Y_vs_d0_O="Sampled0_Y - Sampled0_Old",
                          ...
                          d7_Y_vs_d7_O="Sampled5_Y - Sampled5_Old",
...
{
  tab<-topTable(e, number=nrow(e),coef=i,lfc=1,p.value  = 0.001)
