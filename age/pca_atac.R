...
data$Start=NULL 
data$End=NULL
batches=data.frame(read.table("batches.filtered.atac.txt",header=TRUE,sep='\t'))
...
fit <- lmFit(v)
sv_contribs=coefficients(fit)[,7:15] %*% t(fit$design[,7:15])
v$E=v$E-sv_contribs
filtered=v$E
...
{
  tab<-topTable(e, number=nrow(e),coef=i,lfc=1,p.value  = 0.000001)
