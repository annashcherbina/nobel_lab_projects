...
data=data.frame(read.table('age.rna.counts.txt.expected_count.filtered',header=TRUE,sep='\t',row.names=1))
data$GeneName=NULL 
...
data=data[getVar>0,]
batches=data.frame(read.table("batches.filtered.rna.txt",header=TRUE,sep='\t'))
