...
countData=data.frame(read.table("RSEM.isoforms.expected_count.gt1.nobadreps.tsv",header=T,sep='\t'))
...
for (i in 1:4){
  ...
  #plot the MAplot
  png(paste("isoforms_",conditionsToCompare[i,1],"_vs_",conditionsToCompare[i,2],".png",sep=""))
...
write.table(diffMat,file = "isoforms.diffMat.tsv",sep='\t')
write.table(confidenceMat,file = "isoforms.confidenceMat.tsv",sep='\t')
write.table(fcMat,file = "isoforms.fcMat.tsv",sep='\t')
...
#      png(paste('isoforms',as.character(colData$Sample[i]),"_",as.character(colData$Sample[j]),".png",sep=""))
